...
cell_size = 0.01 # cellule en cm

# ---------------- Fonctions ----------------

def estUnObstacle(coords):
    # Liste des positions à vérifier
    positions = [can_positions, plank_positions_horizontal, plank_positions_vertical]

    # Retourne True si coords est dans l'une des positions, sinon False
    return any(coords in position for position in positions)
...
custom_cmap = ListedColormap([group_colors[key] for key in sorted(group_colors.keys())])
...
grid = np.full((grid_rows, grid_cols), fond_index, dtype=int)
...
def fill_indexes_between_corners2D(corner1, corner2):
    x1, y1 = corner1
    x2, y2 = corner2
...
for obstacle in obstacles:
    ...
    if 0 <= y_idx < grid_rows and 0 <= x_idx < grid_cols:
        grid[y_idx][x_idx] = obstacle_index  # Marquer l'obstacle

# Affichage de la scene sur la grille
scene = fill_indexes_between_corners2D([-0.45, 1], [0.45, 0.55,])
left_ramp = fill_indexes_between_corners2D([-0.85, 1], [-0.45, 0.8])
right_ramp = fill_indexes_between_corners2D([0.45, 1], [0.85, 0.8])
scene_positions = scene + left_ramp + right_ramp

for obstacle in scene_positions:
    y_idx, x_idx = position_to_grid_index(obstacle)
    if 0 <= y_idx < grid_rows and 0 <= x_idx < grid_cols:
        grid[y_idx][x_idx] = scene_index  # Marquer l'obstacle

# Afficher les obstacles pour vérification
for y in range(grid_rows):
    for x in range(grid_cols):
        if grid[y][x] == obstacle_index:
...
def a_star(grid, start, goal):
    ...
    while open_set:
        ...
        if current == goal:
            ...
            path.append(start)
            return path[::-1]  # Retourner le chemin complet

        for dx, dy in [(1, 0), (0, 1), (-1, 0), (0, -1)]:  # Mouvement à 4 directions
            neighbor = (current[0] + dx, current[1] + dy)
            if 0 <= neighbor[0] < grid_rows and 0 <= neighbor[1] < grid_cols:
                if (grid[neighbor[0]][neighbor[1]] == obstacle_index | grid[neighbor[0]][neighbor[1]] == scene_index):
                    continue  # Ignorer les obstacles et la scène
...
grid_with_path[start[0]][start[1]] = start_index  # Marquer la position de départ
grid_with_path[goal[0]][goal[1]] = end_index   # Marquer la position d'arrivée
...
im = ax.imshow(grid_with_path, cmap=custom_cmap, interpolation="nearest", origin="lower")
im
...
if path:
    ...
    for i, step in enumerate(path):
        ...
        for inter_pos in interpolated_positions:
            ...
            grid_with_path[step[0]][step[1]] = position_index  # Marquer temporairement la position
            im.set_data(grid_with_path)
            plt.draw()

        # Réinitialiser la cellule pour éviter des doublons visuels
        grid_with_path[step[0]][step[1]] = fond_index
        plt.pause(0.001)
